...
托管业务覆盖层现已加入不可被放行覆盖的全局工具拦截器、业务预设和精确 Remote 端点名单。普通对话不能通过审批、配置或旧编程预设恢复代码执行。字段规划和站点迁移全部拒绝；通用 Frappe 适配器还拒绝可执行和结构元数据，并校验直接调用。现有业务记录更新保留预览、审批和 Frappe 校验。本项未实现共享登录身份绑定或安全的界面创建执行器。无需密钥的 native-bench-business Web 录制验证实际配置、聊天持久化及拒绝；源码策略和 Python 结构冻结测试覆盖直接绕过尝试。
