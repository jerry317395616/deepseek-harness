...
Host deployments can set `allowedEndpoints` to an exact list of Remote endpoints. The default `all` leaves endpoint selection unrestricted; `[]` denies every Gateway invocation. A list does not accept wildcards or path prefixes. Gateway captures the list for its lifetime and rejects excluded calls with `gateway/forbidden` before resolving descriptors, receiver Contexts, or lookup arguments. The same check covers HTTP unary calls, direct Host calls, and each WebSocket or in-process logical stream; `$events` and `$events/result` each require an explicit entry.
...
- `allowedEndpoints` limits Gateway dispatch, not direct business-Service calls, other Connection channels, exact Fetch routes, tools, files, or operating-system access. It is a Host-wide policy, not user or session ownership. An allowed endpoint retains all authority of its arguments; allowed `$events` forwards the complete configured event selection. Employee deployments still need isolated runtimes and homes, a restricted composition, application ownership checks, and revocation of open connections. Policy replacement requires replacing the Gateway; editing a captured config array does not widen it.
