...
用户可以在模型页面配置 API 密钥、编辑模型列表并添加自定义 pi-ai 路由。提供方、设置和凭据共享一致的视图；密钥值只能写入。首次使用且没有可用提供方时，用户可以配置官方 DeepSeek 凭据，也可以暂时跳过。IONE 部署不挂载官方测试声明。模型页面保留不可用路由、保存失败和只读部署的诊断信息。
