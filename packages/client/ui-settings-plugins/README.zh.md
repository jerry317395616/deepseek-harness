...
本包自带的卡片覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）以及 SearXNG 搜索提供方（`web-search-searxng`）。
...
字段是否被覆盖，取决于它是否出现在原始用户层中，而非取决于它的值；重置会清除该字段，使其重新继承组装层。SearXNG 卡片只保存接口地址，不要求填写提供方密钥。
