...
describe('WebSearchCardController', () => {
  it('reads the SearXNG endpoint directly from the settings section', () => {
  ...
  it('saves a staged endpoint through the settings section', async () => {
    const host = stubSettingsScope<WebSearchSettings>()
    acceptWrites(host)
    const controller = new WebSearchCardController(host.scope)
    ...
    face.edit('baseURL', 'http://127.0.0.1:8088')
    expect(face.hooks.webSearchCard.getSnapshot().dirty).toBe(true)
    face.save()
    await vi.waitFor(() => { expect(host.set).toHaveBeenCalledWith('baseURL', 'http://127.0.0.1:8088') })
  })

  it('can stage a blank endpoint to make the provider inherit its deployment default', () => {
    const host = stubSettingsScope<WebSearchSettings>()
    const controller = new WebSearchCardController(host.scope)
    host.publish({ status: 'ready', writable: true, value: {}, user: {} })
    const face = controller.inject()
  ...
  it('resets an inherited endpoint without any credential service', async () => {
    const host = stubSettingsScope<WebSearchSettings>()
    acceptWrites(host)
    const controller = new WebSearchCardController(host.scope)
    host.publish({
      status: 'ready',
      writable: true,
      base: { baseURL: 'http://127.0.0.1:8088' },
      value: { baseURL: 'https://search.test' },
      user: { baseURL: 'https://search.test' },
    })
    const face = controller.inject()
