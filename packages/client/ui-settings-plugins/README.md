...
The cards this package ships cover the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), and the SearXNG search provider (`web-search-searxng`).
...
A field's presence in the raw user layer — not its value — is what marks it overridden; a reset clears that field so it re-inherits the composition layer. The SearXNG card stores only its endpoint and never asks for a provider credential.
