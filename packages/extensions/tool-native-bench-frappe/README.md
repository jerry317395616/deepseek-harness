...
The read-only [deployment preflight](../../../scripts/employee-deployment-preflight.py) inspects the fixed Child/Harness HTTPS entry and a local systemd runtime without authenticating, reading response bodies or collecting credentials. Run `native-bench/env/bin/python -B deepseek-harness/scripts/employee-deployment-preflight.py` from the deployment root; the default target is the user unit `ione-harness.service`, with explicit `--unit` and `--scope user|system` overrides for a candidate runtime. It reports certificate-verified login redirection, process identity separation, selected sandbox settings and finite resource limits. Missing, changing or failed observations fail closed. It always exits 2 and reports `deployment_approved: false`: even a passing automated baseline requires independent acceptance of routing, authenticated browser behavior, filesystem/credential isolation, privileged-helper separation and operational failure/load behavior. It does not install or enforce restrictions, inspect nginx routing, or certify a multi-user deployment.

<a id="employee-read-broker"></a>
### Employee read broker backend

The optional Linux [read broker](python/employee_read_broker.py) separates a trusted Bench reader from a caller identified by Unix-socket `SO_PEERCRED`. Its private configuration maps each existing, distinct Linux UID to one private renewer configuration and a DocType allowlist. Root, the broker UID, the Bench-owner UID, duplicate employees and protected DocTypes are rejected. Deployment owns the socket's canonical directory and every ancestor; none may be group/world writable or owned by an unrelated user. The socket is mode 0666; its location controls reachability, while the kernel UID binding controls authorization.

Each connection carries exactly one newline-terminated JSON object followed by a write-half close: integer `version: 1`, `operation` and `arguments`. Only `frappe_describe_doctype`, `frappe_list_documents` and `frappe_get_document` are accepted. Identity, paths, executable and scope are not request fields. The broker invokes the existing business reader under the pinned Frappe account, then runs the renewer's identity check before releasing results. Changed identity configuration, failed checks, invalid input and oversized results return a fixed denial; deployment and worker diagnostics are not forwarded.
