...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  const oldText = 'ione harness'
  const sourceNeedle = "'hero.headline': 'ione harness'"
  ...
  try {
    ...
    await page.locator('[data-phase="hero"]').getByText(oldText, { exact: true }).waitFor({ timeout: 15_000 })
