...
describe('web e2e: /goal human transcript presentation', () => {
  ...
  it('completes with Tab and shows the bare input and result without a model turn', async () => {
    ...
    await expect.poll(() => page.locator('[data-phase="hero"]').getByText('ione harness', { exact: true }).count(), {
      timeout: 15_000,
    }).toBe(1)
    ...
    expect(await page.locator('[data-phase="hero"]').getByText('ione harness', { exact: true }).count()).toBe(0)
