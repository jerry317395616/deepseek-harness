...
describe('web e2e: lifecycle & chrome (workspace flow / reload / dark mode)', () => {
  ...
  it('sends the first prompt from the empty-state hero (all modes)', async () => {
    ...
    await expect.poll(() => page.locator('[data-phase="hero"]').getByText('ione harness', { exact: true }).count(), { timeout: 15_000 }).toBe(1)
    const input = page.locator('[data-composer-input]').first()
    await input.waitFor({ timeout: 10_000 })
    if (MODE !== 'record') {
      await page.locator('[data-phase="hero"]').getByText('ione harness', { exact: true }).hover()
